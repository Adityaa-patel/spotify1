...
# # Ensure 'ts' is in datetime format and sorted
# songs_only_df['ts'] = pd.to_datetime(songs_only_df['ts'])
# songs_only_df = songs_only_df.sort_values('ts')
...
# artists = ["Eminem", "Nirvana", "The Beatles"]
# plt.figure(figsize=(12, 6))

# for artist in artists:
#     artist_data = songs_only_df[songs_only_df['master_metadata_album_artist_name'] == artist]
#     weekly_data = artist_data.set_index('ts').resample('W')['ms_played'].sum().reset_index()
#     weekly_data['hours_played'] = weekly_data['ms_played'] / 1000 * 60 * 60
#     plt.plot(weekly_data['ts'], weekly_data['hours_played'],
#              label=artist, alpha=0.4, linewidth=2)

# total_data = songs_only_df.set_index('ts').resample('W')['ms_played'].sum().reset_index()
# total_data['hours_played'] = total_data['ms_played'] / 1000 * 60 * 60
